export default function IconBar({ activeSection, onSelect }) {
  const icons = [
    ...
    { id: 'notifications', icon: '/assets/2222.png', iconSelected: '/assets/2222.png', label: 'Notifications' },
  ]
      ...
      {icons.map(icon => {
        ...
        return (
          <button key={icon.id} onClick={() => onSelect(icon.id)} title={icon.label}
            style={{ width: '36px', height: '36px', borderRadius: isActive ? '12px' : '50%', background: isActive ? '#000000' : 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-radius 0.2s, background 0.2s' }}
