  ...
  return (
    <div style={{ padding: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        ...
        <button onClick={handleNewProject} style={{ padding: '0.6rem 1.2rem', background: '#111', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '500' }}>
          + New Project
        </button>
      </div>
  ...
  return (
    <div onClick={onClick} style={{ background: 'white', border: '1px solid #eee', borderRadius: '10px', padding: '1.25rem', cursor: 'pointer', transition: 'transform 0.15s, box-shadow 0.15s' }}
      ...
      <div style={{ display: 'flex', gap: '0.5rem' }}>
        ...
        <span style={{ fontSize: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: '20px', background: '#e8f0fe', color: '#246e9d' }}>{project.role}</span>
      </div>
    </div>
  )
}
